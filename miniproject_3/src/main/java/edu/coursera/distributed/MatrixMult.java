...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        // Do matrix multiply { same code runs in every process }cd cd
        ...
        if (myRank == 0) {
            ...
            mpi.MPI_Waitall(requests); // like async-await, wait for all asynch

        } else {
            // Other Ranks send their Results to rook rank
            mpi.MPI_Send(c.getValu es(), start * c.getNCols(), (end - start) * c.getNCols(), 0, myRank, mpi.MPI_COMM_WORLD);
